...
export default function Cusines() {
	...
	useEffect(() => {
		fetch("http://localhost:2503/api/cusine")
			.then(res => {
				return res.json()
			})
			.then(data => {
				setCusines(data.document)
			})
	}, [])
					...
					<div>
						...
						<nav aria-label="breadcrumb" className="d-inline-block mt-2 mt-sm-0">
							<ul className="breadcrumb bg-transparent rounded mb-0 p-0">
								...
								<li className="breadcrumb-item text-capitalize active" aria-current="page">Cusine</li>
							</ul>
						</nav>
					</div>

					<div className="mt-4 mt-sm-0">
						<NavLink to="#" className="btn btn-primary" data-bs-toggle="modal" data-bs-target="#add-product">Add Cusine</NavLink>
					</div>
				</div>
							...
							<table className="table table-center bg-white mb-0" id="table">
								<thead>
									<tr>
										...
										<th className="border-bottom p-3" style={{ minWidth: "200px" }}>Cusine Name</th>
								...
								<tbody>
									{
										...
										Cusines.map((cusine, index) => {
											return <Cusine key={index} cusine={Cusines} index={index} />
										})
									}
								</tbody>
							</table>
